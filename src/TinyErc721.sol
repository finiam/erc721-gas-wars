...
import "./TinyERC721Spec.sol";
import "./TinyAccessControl.sol";
import "./TinyAddressUtils.sol";
import "./TinyArrayUtils.sol";
import "./TinyStringUtils.sol";
import "./TinyECDSA.sol";
...
abstract contract TinyERC721 is ERC721Enumerable, ERC721Metadata, MintableERC721, BurnableERC721, AccessControl {
  ...
  constructor(string memory _name, string memory _symbol) {
    ...
    DOMAIN_SEPARATOR = keccak256(
      abi.encode(DOMAIN_TYPEHASH, keccak256(bytes("AliERC721v1")), block.chainid, address(this))
    );
  }
  ...
  function transferFrom(
    ...
    require(
      _from == msg.sender || msg.sender == getApproved(_tokenId) || isApprovedForAll(_from, msg.sender),
      "access denied"
    );
  ...
  function permit(
    ...
    address signer = __deriveSigner(
      abi.encode(PERMIT_TYPEHASH, _owner, _operator, _tokenId, permitNonces[_owner]++, _exp),
      v,
      r,
      s
    );
  ...
  function permitForAll(
    ...
    // update the nonce for that particular signer to avoid replay attack!!! --------------------->>> ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
    address signer = __deriveSigner(
      abi.encode(PERMIT_FOR_ALL_TYPEHASH, _owner, _operator, _approved, permitNonces[_owner]++, _exp),
      v,
      r,
      s
    );
  ...
  function __addLocal(uint256 _tokenId, address _to) internal virtual {
    ...
    tokens[_tokenId] =
      (tokens[_tokenId] & 0x00000000FFFFFFFF000000000000000000000000000000000000000000000000) |
      (uint192(destination.length) << 160) |
  ...
  function __addTokens(
    ...
    for (uint256 i = 0; i < n; i++) {
      // update token global and local indexes, ownership
      tokens[_tokenId + i] =
        (uint224(allTokens.length + i) << 192) |
        (uint192(destination.length + i) << 160) |
        uint160(_to);
    }
  ...
  function __removeLocal(uint256 _tokenId) internal virtual {
    ...
    if (i != source.length - 1) {
      ...
      tokens[sourceId] =
        (tokens[sourceId] & 0x00000000FFFFFFFF00000000FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF) |
        (uint192(i) << 160);
      //  |unused |global | local | ownership information (address)      |
  ...
  function __removeToken(uint256 _tokenId) internal virtual {
    ...
    if (i != allTokens.length - 1) {
      ...
      tokens[lastId] =
        (tokens[lastId] & 0x0000000000000000FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF) |
        (uint224(i) << 192);
      //  |unused |global | local | ownership information (address)      |
